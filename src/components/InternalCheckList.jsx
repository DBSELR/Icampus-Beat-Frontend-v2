...
import {
  getAppData,
  getInternalChecklistReport,
  getInternalChecklistSemesters,
  getInternalChecklistBranches,
} from '../utils/api';
import styles from './InternalCheckList.module.css';

// Derive 2-char regu from regulation (e.g. R20 → 20, 20 → 20)
...
// Normalize course for API (e.g. B.TECH → B.Tech, M.TECH → M.Tech)
...
const InternalCheckList = () => {
  ...
  const fetchSemesters = useCallback(async () => {
    ...
    setLoadError('');
    try {
      const res = await getInternalChecklistSemesters(normalizeCourseForApi(course), regu);
      ...
      console.error('InternalChecklist semesters error:', err);
      setSemesterOptions([]);
  ...
  const fetchBranches = useCallback(async () => {
    ...
    try {
      const res = await getInternalChecklistBranches(normalizeCourseForApi(course), regu);
      if (res?.success && Array.isArray(res?.data)) {
        // Backend SQL: GSUB GRP, GRP ID → GRP = full name (GSUB), ID = short code (GRP). Report API needs grp=short code (CE).
      ...
      console.error('InternalChecklist branches error:', err);
      setBranchOptions([]);
  ...
  const handleView = async () => {
    ...
    setLoadError('');
    try {
      const res = await getInternalChecklistReport(
      ...
      console.error('InternalChecklist report error:', err);
      setReportData(null);
  ...
  const handleDownload = () => {
    ...
    // CSV export of report data if array of rows
    if (Array.isArray(reportData) && reportData.length > 0) {
      ...
      a.href = url;
      a.download = `InternalChecklist_${course}_${examMY}_${filters.semester}_${filters.branch}.csv`;
