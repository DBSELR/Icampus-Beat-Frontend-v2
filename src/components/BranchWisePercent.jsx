...
import styles from './BranchWisePercent.module.css';
import { getAppData, getBranchWisePercentSems, getBranchWisePercentData } from '../utils/api';

const BranchWisePercent = () => {
  ...
  useEffect(() => {
    ...
    getBranchWisePercentSems(course, examMY)
      .then(res => {
  ...
  const handleView = async () => {
    ...
    setTableData([]);
    try {
      const res = await getBranchWisePercentData(course, examMY, regu, semester, isRv);
  ...
  const handleClear = () => {
    setSemester(''); setIsRv(false); setIsSup(false);
    ...
    <div className={styles.container}>
      <div className={styles.panel}>
        <div className={styles.filtersRow}>
          ...
          <div className={styles.checkGroup}>
            <label className={styles.checkLabel}>
              <input type='checkbox' checked={isSup} onChange={e => setIsSup(e.target.checked)} />
              &nbsp;Sup
            </label>
          </div>
      ...
      <div className={styles.reportArea}>
        {tableData.length === 0 && !isLoading && !message.text && (
          <div className={styles.placeholder}>Select Semester and click View to load the report.</div>
        )}
...
export default BranchWisePercent;
