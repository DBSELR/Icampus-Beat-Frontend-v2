...
import styles from './UniversitySubjectData.module.css';
import {
  getAppData,
  getUniversitySubjectDataBatch,
  getUniversitySubjectDataSem,
  getUnivSubjectDataSubjectList,
  getUniversityStudentsData,
} from '../utils/api';
...
const UniversitySubjectData = () => {
  ...
  // Load batch (ExamMY) options on mount
  useEffect(() => {
    if (!course || !regulation) return;
    getUniversitySubjectDataBatch(course, regulation)
      .then(res => {
  ...
  const handleBatchChange = async (val) => {
    ...
    try {
      const res = await getUniversitySubjectDataSem(course, val, regulation);
      if (res.success && res.data)
  ...
  const handleSubjectList = async () => {
    ...
    setTableData([]);
    try {
      const res = await getUnivSubjectDataSubjectList(course, regulation, batch, sem);
      if (res.success && res.data && res.data.length > 0) {
        setTableCols(Object.keys(res.data[0]));
        setTableData(res.data);
        downloadCsv(res.data, `Subject_List_${batch}_Sem${sem}.csv`);
  ...
  const handleStudentsData = async () => {
    ...
    setTableData([]);
    try {
      const res = await getUniversityStudentsData(course, regulation, batch, sem, regSup);
      if (res.success && res.data && res.data.length > 0) {
        setTableCols(Object.keys(res.data[0]));
        setTableData(res.data);
        downloadCsv(res.data, `Students_Data_${batch}_${regSup}_Sem${sem}.csv`);
