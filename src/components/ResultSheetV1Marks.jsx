...
import styles from './ResultSheetV1Marks.module.css';
import { getAppData, getResultSheetSems, getResultSheetData } from '../utils/api';

const ResultSheetV1Marks = () => {
  ...
  useEffect(() => {
    ...
    getResultSheetSems(course, examMY, regu)
      .then(res => {
  ...
  const handleView = async () => {
    ...
    setTableData([]);
    try {
      const res = await getResultSheetData(course, examMY, regu, sem, isReadmit, readmitRegu.trim());
  ...
  const handleDownload = () => {
    ...
    a.href = url; a.download = `ResultSheet_Marks_${sem}.csv`; a.click();
          ...
          {isReadmit && (
            <div className={styles.filterGroup}>
              ...
              <input type='text' value={readmitRegu}
                ...
                className={styles.input} placeholder='e.g. R20' maxLength={10} />
            </div>
          )}
...
export default ResultSheetV1Marks;
