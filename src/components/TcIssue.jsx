...
import styles from './TcIssue.module.css';
import { getTcIssueStudentInfo, postTcIssue } from '../utils/api';

const GENDER_OPTIONS    = ['', 'Male', 'Female', 'Other'];
const CASTE_OPTIONS     = ['', 'OC', 'BC-A', 'BC-B', 'BC-C', 'BC-D', 'BC-E', 'SC', 'ST'];
const YES_NO            = ['', 'YES', 'NO'];
const COMPLETED_OPTIONS = ['', 'COMPLETED', 'NOT COMPLETED'];
...
const TcIssue = () => {
  ...
  const handleRegNoFetch = async () => {
    ...
    setIsFetching(true);
    try {
      const res = await getTcIssueStudentInfo(val);
      if (res.success && res.data) {
        const raw = Array.isArray(res.data) ? res.data[0] : res.data;
        // case-insensitive lookup — handles whatever casing the serializer returns
        ...
        const toDateVal = (s) => {
          if (!s) return '';
          // trim ISO datetime to YYYY-MM-DD for <input type="date">
        ...
        setDateAdmitted(toDateVal(get('dateofadmitted', 'dateadmitted')));
      } else {
  ...
  // Convert YYYY-MM-DD → DD-MM-YYYY (SP uses convert(date, val, 105))
  const toSPDate = (val) => {
    if (!val) return '';
  ...
  const handleSubmit = async () => {
    ...
    if (!dateLeaving)  { showMsg('Please select Date of Leaving.'); return; }
    setIsLoading(true);
    try {
      const req = {
        ...
        mole2,
        religion,
        dateofAdmitted: dateAdmitted,
        scholar:        scholarship,
        courseComplete: courseCompleted,
        higherEdu:      higherEducation,
        dateofLeave:    toSPDate(dateLeaving),
      };
      const res = await postTcIssue(req);
      if (res.success) {
        showMsg(res.message || 'Transfer Certificate issued successfully.', 'success');
      } else {
    ...
    <div className={styles.container}>
      <div className={styles.box}>
        <div className={styles.boxHeader}>
          <h2>Transfer Certificate Issuing</h2>
        </div>
        <div className={styles.boxContent}>
          <div className={styles.formCols}>
            ...
            <div className={styles.formCol}>
              ...
              <Field label="MOLE 1">
                <input type="text" className={styles.inputField} value={mole1}
                  onChange={e => setMole1(e.target.value)} />
              </Field>
              <Field label="MOLE 2">
                <input type="text" className={styles.inputField} value={mole2}
                  onChange={e => setMole2(e.target.value)} />
              </Field>
              <Field label="Religion">
                <input type="text" className={styles.inputField} value={religion}
                  onChange={e => setReligion(e.target.value)} />
              </Field>
              <Field label="Date of Admitted">
                <input type="date" className={styles.inputField} value={dateAdmitted}
                  onChange={e => setDateAdmitted(e.target.value)} />
              </Field>
              <Field label="Scholarship Holder">
                <select className={styles.selectField} value={scholarship}
                  onChange={e => setScholarship(e.target.value)}>
                  {YES_NO.map(o => <option key={o} value={o}>{o || '-- Select --'}</option>)}
                </select>
              </Field>
              <Field label="Course Completed">
                <select className={styles.selectField} value={courseCompleted}
                  onChange={e => setCourseCompleted(e.target.value)}>
                  {COMPLETED_OPTIONS.map(o => <option key={o} value={o}>{o || '-- Select --'}</option>)}
                </select>
              </Field>
              <Field label="Higher Education">
                <select className={styles.selectField} value={higherEducation}
                  onChange={e => setHigherEducation(e.target.value)}>
                  {YES_NO.map(o => <option key={o} value={o}>{o || '-- Select --'}</option>)}
                </select>
              </Field>
              <Field label="No Result Data">
                <label className={styles.checkboxInline}>
                  <input type="checkbox" checked={noResultData}
                    onChange={e => setNoResultData(e.target.checked)} />
                  Student With No Result Data
                </label>
              </Field>
              <Field label="Date of Leaving">
                <input type="date" className={styles.inputField} value={dateLeaving}
          ...
          <div style={{ marginTop: '16px' }}>
            ...
            >
              {isLoading ? 'Issuing...' : 'Transfer Certificate Issue'}
            </button>
          </div>
