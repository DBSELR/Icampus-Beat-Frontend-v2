...
import styles from './IndividualMarksMemo.module.css';
import {
  getAppData,
  getIndividualMarksMemoSems,
  getIndividualMarksMemoBranches,
  getIndividualMarksMemoData,
} from '../utils/api';
...
const IndividualMarksMemo = () => {
  ...
  useEffect(() => {
    ...
    getIndividualMarksMemoSems(course, examMY, regu)
      .then(res => {
        if (res.success && res.data)
          setSemOptions(res.data.map(r => String(r.SEM || r.Sem || r.sem || r)));
      }).catch(() => {});
    getIndividualMarksMemoBranches(course)
      .then(res => {
        if (res.success && res.data)
  ...
  const handleView = async () => {
    ...
    setTableData([]);
    try {
      const reportType = isMarksMemo ? 1 : 2;
      const res = await getIndividualMarksMemoData(regu, examMY, course, sem, branch, regNo, reportType);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      doc.save(`IndividualMarksMemo_${sem}.pdf`);
    } catch (err) {
...
export default IndividualMarksMemo;
